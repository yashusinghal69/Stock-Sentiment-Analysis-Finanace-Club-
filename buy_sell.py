...
data = pd.read_csv('googlestocks with Finbert sentiment to generate plot.csv')
# data = pd.read_csv('stock_sentiment_vader.csv')
# data = data[data['ticker'] == 'SPOT']
# data = pd.DataFrame(data)

# Dataframe editing
data = data.rename(columns={'finbert_sentiment_score': 'sentiment'})
# data = data.rename(columns={'vader_compound': 'sentiment'})
...
print(data)
# data= data[['Date','Close','High','Low','Open','sentiment']]
...
quantstats.reports.html(returns, output='stats.html', title='Google Sentiment')
